...
// Convert: best, effiecient
// Notice!!!:   since, we are shifting number left, we do not have to use unsigned int.
//       !!!:   'while (decimal != 0)' doesn't get stuck in an infinit loop
...
// from geeks:  not effiecient, unnecessary conmplicated algorithem, and uses extra space.
//              also, result is a reverse number string
// Notice!!!:   since, we are shifting number right, we must use unsigned int,
//       !!!:   so 'while (decimal > 0)' doesn't get stuck in an infinit loop
string DecimalToBinary_geeks( int decimal )
{
    string binaryString = "";
    // array to store binary number
    int binaryNum[32] = {0};    // Taher!!!: we really don't need this

    // counter for binary array
    int i = 0;
    while (decimal > 0) {   // Taher!!!: does not work, when decimal < 0. returns empty string
    ...
    // Taher!!!: I added this line to reverse string
